        ...
        <div style={{
          ...
          <div style={{ marginBottom: "12px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
              {DAYS.map((day, dayIdx) => (
                <div key={dayIdx}>
                  <p style={{ fontSize: "11px", fontWeight: "600", color: "#6b7280", marginBottom: "4px" }}>{day}</p>
                  {HOURS.map(time => {
                    const isSelected = form.schedule.find(s => s.day === dayIdx && s.time === time);
                    return (
                      <button
                        key={`${dayIdx}-${time}`}
                        onClick={() => toggleSchedule(dayIdx, time)}
                        disabled={saving}
                        style={{
                          width: "100%",
                          padding: "6px 8px",
...
function AttendanceTab({ students, records, setRecords, loadingData }) {
  ...
  async function updateAttendance(classKey, status) {
    if (!user) return;
    setSaving(true);
    ...
    try {
      ...
      if (status === null) {
        ...
        await setDoc(recordDoc, { status }, { merge: true });
      }
      ...
      setRecords(prev => {
        ...
        if (existing >= 0) {
          ...
          newRecords[existing] = { ...prev[existing], status };
          return newRecords;
        }
        return [...prev, { key: classKey, status }];
      });
    } catch (error) {
          ...
          {classesForDate.map(cls => (
            <div key={cls.key} style={{
              ...
              <div>
                ...
                <p style={{ fontSize: "12px", color: "#9ca3af", margin: "0" }}>{cls.time}</p>
              </div>
              <div style={{ display: "flex", gap: "6px" }}>
                <button
                  onClick={() => updateAttendance(cls.key, cls.status === "present" ? null : "present")}
                  disabled={saving}
                  style={{
                    padding: "8px 12px",
                ...
                <button
                  onClick={() => updateAttendance(cls.key, cls.status === "absent" ? null : "absent")}
                  disabled={saving}
                  style={{
                    padding: "8px 12px",
...
function ReportsTab({ students, records, payments, loadingData }) {
  ...
  monthRecords.filter(r => r.status === "present").forEach(record => {
    ...
    if (student && studentBreakdown[studentId]) {
      ...
      studentBreakdown[studentId].revenue += student.pricePerClass;
    }
  });
